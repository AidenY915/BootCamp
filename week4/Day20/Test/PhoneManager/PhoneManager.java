...
class PhoneBook implements Constants {
	...
	public void add(int type, String[] input) {
		PhoneInfo tmp = null;
		switch (type) {
			case NORMAL : tmp = new PhoneInfo(input[0], input[1]); break;
			case UNIV : tmp = new PhoneUnivInfo(input[0], input[1], input[2], Integer.parseInt(input[3])); break;
			case COMPANY : tmp = new PhoneCompanyInfo(input[0], input[1], input[2]); break;
		}
