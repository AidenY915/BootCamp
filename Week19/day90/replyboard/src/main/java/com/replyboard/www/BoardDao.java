...
public class BoardDao {
	...
	public void updateBoard(BoardDto dto) {
		try(Connection conn = dataSource.getConnection()){
			String updateQuery = "UPDATE " + TABLE + " SET writer = ?, title = ?, content = ? WHERE no = ?";
			PreparedStatement updateStmt = conn.prepareStatement(updateQuery);
			updateStmt.setString(1, dto.getWriter());
			updateStmt.setString(2, dto.getTitle());
			updateStmt.setString(3, dto.getContent());
			updateStmt.setInt(4, dto.getNo());
