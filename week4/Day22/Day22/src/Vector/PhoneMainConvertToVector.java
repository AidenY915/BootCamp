...
class PhoneBook
{
	...
	public void inputPhoneInfo(PhoneInfo pInfo)
	{
		...
		for(i=0;i<pInfoVec.size();i++)
		{
			...
			{
					pInfoVec.add(null);					//size를 먼저 늘려줘야 함. 그래야 푸쉬할 공간이 생김.
				for(j=pInfoVec.size() - 2;j>=i;j--)		//size() - 1이 마지막 인덱스고, 현재 비어있음.
				{
					pInfoVec.set(j+1, pInfoVec.get(j));
				}
				break;
			}
		}
...
class PhoneMainConvertToVector
{		
