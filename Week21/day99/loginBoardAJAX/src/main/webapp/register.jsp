...
<body>
	<form method="post" action="RegisterOK.do">
		...
		<div>
			<div>주소</div>
			<input type="text" id="postcode" name="postNum" placeholder="우편번호">
			<input type="button" name='postNum' onclick="execDaumPostcode()"
				value="우편번호 찾기"
	...
	<script>
		...
		req.addEventListener("readystatechange", function() {
			...
			const rslt = JSON.parse(req.response);
			if(rslt.validation === 'true'){
				document.querySelector("#idCheckMessage").innerHTML = "사용가능한 아이디입니다.";
			}
			else {
				document.querySelector("#idCheckMessage").innerHTML = "사용불가능한 아이디입니다.";
			}
		});
		const idCheck = function() {
			let id = document.getElementsByName('id')[0].value;	
			req.open("post", "idCheck.do?id=" + id);
			req.send();
		}
	</script>
</body>
